...
class User:
    ...
    def create_or_update_user(userinfo):
        ...
        user = None
        try:
            ctx = {'fields': "*", 'conds' : f"username=\"{userinfo['username']}\""}
            user_record = store.query(context=ctx)
            if not user_record:
            ...
            ctx = {
                'username': f"\"{user['username']}\"", 'password': f"\"{user['password']}\"", 'updated_at': f"{user['updated_at']}", 
                'created_at': f"{user['created_at']}", 'expires_at': f"{user['expires_at']}", 'active': f"{user['active']}", 
                'logins_count': f"{user['logins_count']}", 'su': f"{user['su']}"
            }
    ...
    def get_user(username):
        ...
        try:
            ...
            userinfo = store.query(context=ctx)
        except Exception as ex:
            raise DatabaseError({
